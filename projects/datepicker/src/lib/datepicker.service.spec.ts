...
xdescribe("DatePickerService", () => {
  ...
  it("should open the date picker dialog", (done: () => unknown) => {
    ...
    setTimeout(() => {
      const cancelButton = fixture.debugElement.query(
        By.css(".mdl-datepicker-cancel-button")
      ).nativeElement;
      expect(cancelButton.textContent).toBe("Cancel");

      const okButton = fixture.debugElement.query(
        By.css(".mdl-datepicker-ok-button")
      ).nativeElement;
  ...
  it("should open the date picker dialog with a provided date", () => {
    ...
    const yearEl = fixture.debugElement.query(
      By.css(".mdl-datepicker-header-year")
    ).nativeElement;
    expect(yearEl.textContent).toBe("2017");

    const dayEl = fixture.debugElement.query(
      By.css(".mdl-datepicker-header-day-month")
    ).nativeElement;
