...
describe("DatePickerDialogComponent", () => {
  ...
  it("should call hide with null on cancel", () => {
    const dialogRef: MdlDialogReference =
      TestBed.inject<MdlDialogReference>(MdlDialogReference);
  ...
  it("should call hide with actual date on ok", () => {
    ...
    expect(dialogRef.hide).toHaveBeenCalledWith(
      fixture.componentInstance.mDate.toDate()
    );
  });

  it("should show the current date - because we did not provide a date", () => {
    expect(
      fixture.componentInstance.mDate.isSame(moment(), "day")
    ).toBeTruthy();
    expect(
      fixture.componentInstance.mCurrentMonth.isSame(
        fixture.componentInstance.mDate,
        "day"
      )
    ).toBeTruthy();
  });
  ...
  it("should create an array with the week days for dow=1 locale", () => {
    fixture.componentInstance.mCurrentMonth = moment("2017-01-01");
    expect(fixture.componentInstance.monthGridWeekDays).toEqual([
      "Mo",
      "Tu",
      "We",
      "Th",
  ...
  it("should create an array with all days for the current month", () => {
    ...
    expect(day1.day.isSame(moment("2017-03-27"), "day")).toBeTruthy(
      "should be the 2017-03-27"
    );
    expect(fixture.componentInstance.isActualDate(day1.day)).toBeFalsy(
      "first day is not the actual"
    );
    expect(day1.isCurrentMonth).toBeFalsy(
      "first day is not in the current month"
    );

    // the third week the 2snd day ist 2017-04-10
    const dayActual = gridWithDays[2].days[0];
    expect(dayActual.day.isSame(moment("2017-04-10"), "day")).toBeTruthy(
      "should be 2017-04-10"
    );
    expect(fixture.componentInstance.isActualDate(dayActual.day)).toBeTruthy(
      "the 10. is the actual day"
    );
    expect(dayActual.isCurrentMonth).toBeTruthy(
      "the 10 is in the current month"
    );

    const lastDayOfTheMonth = gridWithDays[4].days[6];
    expect(
      lastDayOfTheMonth.day.isSame(moment("2017-04-30"), "day")
    ).toBeTruthy("should be 2017-04-30");
    expect(
      fixture.componentInstance.isActualDate(lastDayOfTheMonth.day)
    ).toBeFalsy("30. is not the actual day");
    expect(lastDayOfTheMonth.isCurrentMonth).toBeTruthy(
      "the 30 is in the current month"
    );
  });
  ...
  it("should create an array with all days for the december 2018", () => {
    ...
    const day1 = gridWithDays[0].days[0];
    expect(day1.day.isSame(moment("2018-11-26"), "day")).toBeTruthy(
      "should be the 2018-11-26"
    );

    const lastDayOfTheMonth = gridWithDays[5].days[0];
    expect(
      lastDayOfTheMonth.day.isSame(moment("2018-12-31"), "day")
    ).toBeTruthy("should be 2018-12-31");
  });
});
