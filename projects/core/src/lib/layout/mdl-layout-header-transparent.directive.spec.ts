...
@Component({
  ...
  template:
    "<mdl-layout><mdl-layout-header mdl-layout-header-transparent>x</mdl-layout-header></mdl-layout>",
})
...
describe("Component: MdlLayoutHeaderTransparent", () => {
  ...
  it("should add the css class mdl-layout__header--transparent to the header element", () => {
    ...
    expect(
      headerDebugElement.classList.contains("mdl-layout__header--transparent")
    ).toBe(true);
  });
});
